...
# <a name="explore-text-analytics"></a>Изучение текстовой аналитики
...
В состав Microsoft Azure *Cognitive Services* входит *языковая служба* для анализа текста, в которой реализованы некоторые готовые возможности NLP, включая поиск ключевых фраз в тексте и классификацию текста на основе тональности.
...
Для использования языковой службы можно создать ресурс **Язык** или **Cognitive Services**.
...
### <a name="get-the-key-and-endpoint-for-your-cognitive-services-resource"></a>Получение ключа и конечной точки для ресурса Cognitive Services
...
1. Не стоит волноваться по поводу содержимого кода. На портале Azure перейдите к ресурсу Cognitive Services Azure. Затем выберите страницу **Ключи и конечные точки** на панели слева. Скопируйте ключи и конечную точку со страницы и вставьте в редакторе кода, заменив значения заполнителей **YOUR_KEY** и **YOUR_ENDPOINT** соответственно ключом и конечной точкой.

    > **Совет** Вы можете настроить области **Ключи и конечная точка** и **Редактор** на экране с помощью разделительной полосы.

    ![Найдите вкладку "Ключ и конечная точка" в левой области ресурса Cognitive Services.](media/analyze-text-language-service/key-endpoint-support.png)
...
1. В правом верхнем углу панели редактора нажмите кнопку **...**, чтобы открыть меню, и выберите **Сохранить**, чтобы сохранить свои изменения. Затем снова откройте меню и выберите пункт **Закрыть редактор**.
    ...
    Пример клиентского приложения будет использовать языковую службу Cognitive Services для определения языка отзывов, извлечения из них ключевых фраз, определения тональности и извлечения известных сущностей.
