...
1. На странице **Создание языка** укажите параметры ниже.
    ...
    > **Примечание** Если вы уже подготовили ресурс Когнитивный поиск Azure уровня "Бесплатный **"**, квота может не позволить вам создать еще один ресурс. В этом случае выберите уровень, отличающийся от **Бесплатного (F)** .
...
1. В верхней части страницы щелкните **Развернуть**. В диалоговом окне появится запрос на развертывание проекта. Выберите **Развернуть**.
...
1. В портал Azure создайте бот веб-приложения. (Вы можете увидеть предупреждающее сообщение проверка, что источник шаблона является надежным. Вам не нужно предпринимать никаких действий для этого сообщения.) Для продолжения обновите следующие параметры:

    - **Сведения о проекте**
        - **Подписка**. *Ваша подписка Azure*
        - **Группа ресурсов**: *группа ресурсов, в которой содержится ваш языковой ресурс*
    - **Сведения об экземпляре**
        - **Расположение группы ресурсов**: *то же расположение, что и языковая служба*.
    - **Служба Azure Bot**
        - **Дескриптор бота**: *уникальное имя бота* (*предварительно заполненное*)
    - **Выбор ценовой категории**
        - **Ценовая категория**: Бесплатный (F0) (может потребоваться выбрать *Изменить план*)
    - **Microsoft App ID (Идентификатор приложения Майкрософт)**
        - **Тип создания**: *выберите Создать управляемое удостоверение, назначаемое пользователем*. 

5. Выберите **Далее: >веб-приложения ** , чтобы продолжить обновление параметров. 
    - **Служба приложений**
    ...
    - **Параметры приложения**
        - **Ключ языкового ресурса**. *Необходимо скопировать ключ языкового ресурса и вставить его сюда.* 
        
        > **Примечание** Чтобы перейти к ключу языкового ресурса, откройте [https://portal.azure.com](https://portal.azure.com?azure-portal=true). На домашней странице щелкните *Группы ресурсов* и найдите группу ресурсов, в которую вы создали языковой ресурс. Выберите языковой ресурс и перейдите в его меню слева. Затем выберите *Ключи и конечная точка*. Скопируйте один из ключей. 

    -  
        - **Имя языкового проекта**: MargiesTravel
        - **Имя узла конечной точки языковой службы**: *предварительно заполнено конечной точкой языковой службы*.
    - **Сведения о языковой службе**
        - **Идентификатор подписки**: *предварительно заполнен идентификатором подписки*.
        - **Имя группы ресурсов**: *предварительно заполнено именем группы ресурсов*.
