...
# <a name="explore-knowledge-mining"></a>Изучение интеллектуального анализа знаний
...
## <a name="azure-resources-needed"></a>Необходимые ресурсы Azure
...
- Ресурс **Cognitive Services**, который предоставляет службы ИИ для навыков, которые решение для поиска может использовать для обогащения данных в источнике с помощью формируемой ИИ аналитикой.

    > **Примечание** Когнитивный поиск Azure и его ресурсы должны находиться в одном расположении.
...
### <a name="create-an-azure-cognitive-search-resource"></a>Создание ресурса *Когнитивный поиск Azure*
...
Вам потребуется подготовить ресурс **Cognitive Services**, находящийся в том же расположении, что и ресурс Когнитивного поиска Azure. Решение для поиска будет использовать этот ресурс для обогащения данных в хранилище с помощью генерируемой ИИ аналитики.

1. Вернитесь на домашнюю страницу портала Azure и нажмите кнопку **+ Создать ресурс**, выполните поиск по запросу *Cognitive Services* и создайте ресурс **Cognitive Services** со следующими параметрами: 
    - **Подписка**: *ваша подписка Azure*.
...
1. Найдите *учетную запись хранения*, а затем создайте ресурс **Учетная запись хранения**, задав такие параметры: 
    - **Подписка**: *ваша подписка Azure*.
    - **Группа ресурсов**: *группа ресурсов, в которой находятся Когнитивный поиск Azure и его ресурсы*.
...
## <a name="upload-documents-to-azure-storage"></a>Отправка документов в хранилище Azure

1. В созданной учетной записи хранения Azure в области меню слева выберите **Контейнеры**.
...
## <a name="index-the-documents"></a>Индексация документов
...
1. В разделе **Подключение Cognitive Services** выберите ресурс Cognitive Services.  
...
1. В разделе **Сохранение обогащений в хранилище знаний** установите такие флажки:
    ...
    > 1. Нажмите **+ Контейнер**, чтобы создать контейнер с именем **knowledge-store** и уровнем конфиденциальности "частный", а затем нажмите **Создать**.

    > 1. Выберите контейнер **knowledge-store** и нажмите кнопку **Выбрать** в нижней части экрана.

1. Выберите **Проекты BLOB-объектов Azure: документ**. Появится параметр *Имя контейнера* с автоматически подставленным значением *knowledge-store*. Не изменяйте имя контейнера.
...
1. По завершении выберите **Далее: Создание индексатора**.
...
1. Теперь выполним фильтрацию по расположению. Введите `search=$filter=locations eq 'Chicago'` в поле **Строка запроса**, а затем выберите **Поиск**. Запрос выполняет поиск по всем документам в индексе и отфильтровывает проверки с расположением в Чикаго.

1. Теперь выполним фильтрацию по тональности. Введите `search=$filter=sentiment eq 'negative'` в поле **Строка запроса**, а затем выберите **Поиск**. Запрос выполняет поиск по всем документам в индексе и отфильтровывает проверки с отрицательной тональностью.
...
## <a name="review-the-knowledge-store"></a>Изучение хранилища знаний
