...
1. Измените имя черновика (**Pipeline-Created-on-* date***) на **Обучение модели цен на автомобили**.
...
1. Щелкните правой кнопкой мыши (клавиша Ctrl+щелчок на компьютере Mac) набор данных **Данные о ценах на автомобили (необработанные)** на холсте и выберите **Предварительный просмотр данных**.
...
1. Закройте окно **Вывод данных**, чтобы можно было увидеть набор данных на холсте. Он будет выглядеть приблизительно следующим образом:
...
1. Нажмите **Сохранить** и закройте окно свойств.

В оставшейся части этого упражнения вы выполните шаги для создания конвейера, который выглядит следующим образом:
...
1. Дважды щелкните модуль **Очистка отсутствующих данных** и выберите **Изменить столбец** на панели справа. Затем в окне **Столбцы для очистки** выберите **С правилами**, в списке **Включить** выберите **Имена столбцов**, в поле имен столбцов введите **bore**, **stroke** и **horsepower**, как показано ниже:
...
1. На странице **Основные** выберите **Создать** и задайте имя эксперимента для **mslearn-auto-training**, а затем нажмите **Далее** .
...
1. Подождите несколько минут, пока не завершится выполнение. Вы можете проверить состояние задания, выбрав **Задания** в разделе **Активы**. Здесь выберите задание **Обучение модели цен на автомобили**. Здесь вы можете увидеть, когда задание будет завершено. После завершения задания набор данных готов для обучения модели.

1. Перейдите в меню слева. В разделе **Разработка** выберите **Конструктор**. Затем выберите ваш конвейер *Обучение модели цен на автомобили* в списке **Конвейеров**.
...
1. Убедитесь, что в меню слева выбран **Конструктор**, и вы вернулись в конвейер **Обучение модели цен на автомобили**.
...
1. Выберите **Настроить и Отправить** и запустите конвейер, используя существующий эксперимент с именем **mslearn-auto-training**.

1. Выполнение эксперимента займет около 5 минут. Вернитесь на страницу **Задания** и выберите последнее выполнение задания **Обучение модели цен на автомобили**.

1. Когда выполнение эксперимента завершится, щелкните правой кнопкой мыши модуль **Оценка модели** и выберите **Предварительный просмотр данных**, а затем **Оцененный набор данных** для просмотра результатов.
...
1. Выполнение эксперимента займет несколько минут. Вернитесь на страницу **Задания** и выберите последнее выполнение задания **Обучение модели цен на автомобили**.
...
1. Переименуйте новый конвейер в **Прогнозирование цен на автомобиль**, а затем просмотрите новый конвейер. Он содержит входные данные веб-службы для новых отправляемых данных, а также выходные данные веб-службы для возврата результатов. Некоторые преобразования и этапы обучения являются частью этого конвейера. Обученная модель будет использоваться для оценки новых данных.
    ...
    Вы внесете следующие изменения в конвейер вывода, как описано в следующих шагах:
...
1. При использовании конвейера вывода предполагается, что новые данные соответствуют схеме исходных данных обучения, то есть набор данных **Данные о ценах на автомобиль (необработанные)** из конвейера обучения включен. Однако эти входные данные содержат метку **цена**, которая не включается интуитивно в данные нового автомобиля, для которого еще не сформирован прогноз. Удалите этот модуль и замените его модулем **Ввод данных вручную** из раздела **Ввод и вывод данных**.
...
1. Выходные данные модуля **Оценка модели** включают все компоненты входных данных, а также прогнозируемую метку. Чтобы изменить выходные данные, чтобы они включали только прогноз:
    ...
    ```
>**Примечание**. Копирование и вставка могут вводить пробелы в скрипт Python, которые не должны быть там. Дважды проверьте, нет ли пробела перед словом *import* или *def* или *return*. Убедитесь, что перед *scored_results* и *scored_results.rename()* есть один отступ табуляции.

1. Подключите выходные данные модуля **Оценка модели** к вводу **Набор данных1** (крайний слева) **Выполнение сценария Python**.

1. Подключите вывод **Набора данных результатов** (слева) модуля **Выполнение сценария Python** к модулю **Входных данных веб-службы**.
...
1. Вернитесь на страницу **Задания** и выберите последнее выполненное задание **Обучение модели цен на автомобили** (то, которое связано с *экспериментом mslearn-auto-inference experiment*).

1. После завершения конвейера щелкните правой кнопкой мыши модуль **Выполнение сценария Python**. Щелкните **Предварительный просмотр данных**, а затем **Результирующий набор данных**, чтобы просмотреть прогнозируемые цены для трех автомобилей, предоставленных во входных данных.
...
1. На странице выполнения задания **Прогноз цены на автомобиль** выберите **Развернуть** в верхней строке меню.
...
1. На экране конфигурации выберите **Развернуть новую конечную точку для прогнозирования в реальном времени** и примените следующие параметры:
    - **Имя**: predict-auto-price
    - **Описание**: "Регрессия цен на автомобили"
    - **Тип вычислений**: Экземпляр контейнера Azure

1. Выберите **Развернуть** и подождите несколько минут, пока выполняется развертывание веб-службы.
...
1. Когда откроется конечная точка **predict-auto-price**, перейдите на вкладку **Тест**, где мы выполним тестирование модели с новыми данными. Удалите текущие данные из раздела **Входные данные для тестирования конечной точки для прогнозирования в реальном времени**. Скопируйте и вставьте в раздел данных приведенные ниже данные:  
