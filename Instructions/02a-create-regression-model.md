...
## Создание конвейера в конструкторе и добавление набора данных
...
1. Щелкните правой кнопкой мыши (ctrl+click on a Mac) **набор данных цен на автомобили (необработанные)** на холсте и выберите **предварительный просмотр данных**.
...
1. Нажмите кнопку **"Сохранить** " и закройте окно владельцев.

В остальной части этого упражнения вы узнаете, как создать конвейер, который должен выглядеть следующим образом:
...
1. Дважды щелкните **модуль "Очистить отсутствующие данные** " и в области справа выберите **"Изменить столбец**". Затем в окне **Столбцы для очистки** выберите **С правилами**, в списке **Включить** выберите **Имена столбцов**, в поле имен столбцов введите **bore**, **stroke** и **horsepower**, как показано ниже:
...
1. **На странице "Основы"** выберите **"Создать"** и задайте имя эксперимента для **mslearn-auto-training**, а затем нажмите кнопку **"Далее**".
...
1. Подождите несколько минут, пока не завершится выполнение. Вы можете проверка состояние задания, выбрав **"Задания**" в разделе **"Активы**". Выберите задание автообучения **** цен. Здесь вы можете увидеть, когда задание завершено. После завершения задания набор данных теперь готов к обучению модели.

1. Перейдите в меню слева. В разделе **"Создание"** выберите **конструктор**. Затем выберите *конвейер автоматического обучения* цен в списке **конвейеров**.
...
1. Убедитесь, что в меню **слева выбран конструктор** , и вы вернулись в **конвейер автообучения** цен.
...
1. Выберите " **Настроить" и "Отправить**" и запустите конвейер с помощью существующего эксперимента с именем **mslearn-auto-training**.

1. Выполнение эксперимента займет около 5 минут. Вернитесь на **страницу "Задания** " и выберите последнее **выполнение задания автообучения** цен.

1. После завершения выполнения эксперимента щелкните правой кнопкой мыши **модуль "Оценка модели"** и выберите **предварительный просмотр данных** , а затем **набор данных** scored, чтобы просмотреть результаты.
...
Одним из способов оценки модели регрессии является сравнение прогнозируемых меток с фактическими метками в наборе данных проверки, удерживаемом во время обучения. Также можно сравнить производительность нескольких моделей.
...
1. Выполнение эксперимента займет несколько минут. Вернитесь на **страницу "Задания** " и выберите последнее **выполнение задания автообучения** цен.
...
1. В раскрывающемся списке **конвейера вывода** создания выберите **конвейер** вывода в режиме реального времени. Через несколько секунд откроется новая версия конвейера с именем **Обучение модели цен на автомобили — вывод в реальном времени**.
...
1. Переименуйте новый конвейер в **Прогнозирование цен на автомобиль**, а затем просмотрите новый конвейер. Он содержит входные данные веб-службы для новых отправляемых данных, а также выходные данные веб-службы для возврата результатов. Некоторые преобразования и этапы обучения являются частью этого конвейера. Обученная модель будет использоваться для оценки новых данных.
    ...
    Чтобы внести следующие изменения в конвейер вывода, сделайте следующее:
...
1. При использовании конвейера вывода предполагается, что новые данные соответствуют схеме исходных данных обучения, то есть набор данных **Данные о ценах на автомобиль (необработанные)** из конвейера обучения включен. Однако эти входные данные содержат метку **цена**, которая не включается интуитивно в данные нового автомобиля, для которого еще не сформирован прогноз. Удалите этот модуль и замените его **модулем Ввод данных вручную** из **раздела "Входные и выходные данные** ".
...
1. Выходные данные модуля **Оценка модели** включают все компоненты входных данных, а также прогнозируемую метку. Чтобы изменить выходные данные, чтобы они включали только прогноз:
    ...
    ```
>**Примечание**. Копирование и вставка могут вводить пробелы в скрипт Python, который не должен быть там. Дважды проверка, что нет места перед *импортом* или *дефированием* или *возвратом*. Убедитесь, что перед scored_results* и scored_results.rename()*есть одна вкладка**.

1. Подключение выходные данные из **Модуль оценки модели** в вводе ****набора данных1** (слева) скрипта Python**.

1. ** ПодключениеРезультирующий набор** данных (слева) модуля "Выполнение скрипта **** Python" в **модуль вывода** веб-службы.
...
1. Вернитесь на **страницу "Задания**" и выберите последнее **выполнение задания автообучения** цен (одно из них связано с экспериментом* автоматического *вывода mslearn).

1. После завершения конвейера щелкните правой кнопкой **мыши модуль "Выполнить скрипт** Python". Щелкните **Предварительный просмотр данных**, а затем **Результирующий набор данных**, чтобы просмотреть прогнозируемые цены для трех автомобилей, предоставленных во входных данных.
...
1. **На странице запуска задания "Прогноз автоцены"** выберите **"Развернуть**" в верхней строке меню.
...
1. Выберите **"Развернуть** " и подождите несколько минут, пока веб-служба будет развернута.

## Тестирование службы

1. На странице **Конечные точки** откройте конечную точку в реальном времени **predict-auto-price**.

    ![Снимок экрана: расположение параметра "Конечные точки" на панели слева.](media/create-regression-model/endpoints-lab.png)
