...
1. Выберите **Сохранить** и закройте окно с собственными узлами.

В оставшейся части этого упражнения вы выполните шаги по созданию конвейера, который должен выглядеть следующим образом:
...
1. Дважды щелкните модуль **Очистка отсутствующих данных** и в области справа выберите **Изменить столбец**. Затем в окне **Столбцы для очистки** выберите **С правилами**, в списке **Включить** выберите **Имена столбцов**, в поле имен столбцов введите **bore**, **stroke** и **horsepower**, как показано ниже:
...
1. Подождите несколько минут, пока не завершится выполнение. Вы можете проверка состояние задания, выбрав **Задания** в разделе **Активы**. Выберите задание **Автоматическое обучение цен** .

Теперь набор данных готов для обучения модели. Закройте вкладку сведений о задании, чтобы вернуться к конвейеру.
...
1. Выберите **Настроить & Отправить** и запустите конвейер с помощью существующего эксперимента **с именем mslearn-auto-training**.
...
1. После завершения эксперимента щелкните правой кнопкой мыши модуль **Score Model (Оценка модели** ) и выберите **Preview data (Предварительный просмотр данных** ), а затем **— Scored dataset (Оцененный набор данных** ), чтобы просмотреть результаты.
...
Одним из способов оценки регрессии модели является сравнение прогнозируемых меток с фактическими метками в проверочном наборе данных, который был удержан во время обучения. Также можно сравнить производительность нескольких моделей.
...
1. После завершения эксперимента щелкните правой кнопкой мыши модуль **Evaluate Model (Оценка модели** ) и выберите **Preview data (Предварительный просмотр данных** ), а затем **— Evaluation results (Результаты оценки**).
