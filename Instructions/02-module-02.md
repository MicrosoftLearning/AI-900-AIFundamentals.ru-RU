...
## Включение бессерверных вычислений
...
1. Включите функцию "Интерактивный интерфейс для отправки заданий обучения с помощью бессерверных вычислений".

![Снимок экрана: включение функции бессерверных вычислений.](../instructions/media/use-automated-machine-learning/enable-serverless-compute.png)
...
Обратите внимание, что в разделе *Просмотр дополнительных параметров конфигурации* находится раздел *Ограничения* . Разверните раздел , чтобы настроить параметры:
        - **Время ожидания (в минутах):** 30 — *задание завершается не более чем через 30 минут.*
        ...
        - **Вычисление**: здесь не требуется никаких изменений
        - Нажмите кнопку **Далее**
...
1. Подождите, пока **состояние развертывания** изменится на **Исправно** . Это может занять несколько минут.

## Тестирование развернутой службы

Теперь можно протестировать развернутую службу.

1. На странице **конечной точки predict-rentals** в режиме реального времени откройте вкладку **Тест** .
