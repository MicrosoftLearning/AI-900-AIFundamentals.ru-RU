...
1. Вернитесь к конвейеру, выбрав **Конструктор** в меню слева. На странице **Designer** выберите проект конвейера **Обучение кластеризации пингвинов**.
...
1. Перейдите на вкладку *Профиль* , отметив, что вы можете видеть распределения различных столбцов в виде гистограмм.
...
1. Закройте страницу **DataOutput** , чтобы увидеть набор данных на холсте конвейера.
...
1. Выберите **Сохранить**, а затем закройте меню **Выбор столбцов в наборе данных** , чтобы вернуться на холст конструктора.

1. В **библиотеке активов** найдите модуль **Очистка отсутствующих данных** и поместите его на холст под модулем **Select columns in dataset (Выбор столбцов в наборе данных** ) и подключите их следующим образом:
...
1. Дважды щелкните модуль **Очистка отсутствующих данных** и в области параметров справа выберите **Изменить столбец**. Затем в окне **Столбцы для очистки** выберите **С правилами** и добавьте **Все столбцы** следующим образом:
...
1. Выберите **Сохранить** , а затем в области параметров задайте следующие параметры конфигурации:
...
1. Выберите **Сохранить** , а затем закройте параметры модуля **Нормализация данных** , чтобы вернуться на холст конструктора.
...
1. На странице **Основные сведения** выберите **Создать** и задайте для эксперимента имя **mslearn-penguin-training** , а затем нажмите кнопку **Далее** .
...
1. Дождитесь, пока он завершится. Это может занять 5 минут или больше. Вы можете проверка состояние задания, выбрав **Задания** в разделе **Активы**. Затем выберите задание **Обучение кластеризации пингвинов** .
...
1. Щелкните правой кнопкой мыши модуль **Нормализация данных** , выберите **Предварительный просмотр данных**, а затем — **Преобразованный набор данных** , чтобы просмотреть результаты.
...
1. Вернитесь на страницу **Designer** и откройте черновик конвейера **Обучение кластеризации пингвинов**.
...
1. Модель, для которой выполняется обучение, будет использовать компоненты для группирования данных в кластеры, поэтому нам нужно обучить модель с помощью алгоритма *кластеризации*. В **библиотеке активов** найдите и поместите модуль **кластеризации K-Средних** на холст слева от модуля **Разделение данных** и над модулем **Обучение модели кластеризации** . После этого подключите область его выходных данных к области входных данных **Необученная модель** (слева) модуля **Модель кластеризации**.
...
1. Алгоритм *K-средние* группирует элементы в заданное число кластеров — это значение называется ***K***. Выберите модуль **Кластеризация K-средних** и на панели справа задайте для параметра **Число центроидов** значение **3**.
    ...
    > 5. Повторяющиеся шаги c. и d. пока выделение кластера не стабилизируется или не завершится указанное количество итераций.
...
1. Выберите **Настроить & Отправить** и запустите конвейер, используя существующий эксперимент **с именем mslearn-penguin-training** в вычислительном кластере.

1. Дождитесь, пока он завершится. Это может занять 5 минут или больше. Проверьте состояние задания, выбрав **Задания** в разделе **Активы**. Выберите последнее задание **"Обучение кластеризации пингвинов** ".

1. Когда выполнение эксперимента завершится, щелкните правой кнопкой мыши модуль **Назначение данных кластерам** , выберите **Предварительный просмотр данных** , а затем выберите **Набор данных результатов** , чтобы просмотреть результаты.
...
1. На странице **Designer** откройте проект конвейера **Обучение кластеризации пингвинов**.
...
1. Щелкните правой кнопкой мыши модуль **Evaluate Model (Оценка модели** ), выберите **Предварительный просмотр данных**, а затем — **Результаты оценки**. В каждой строке изучите следующие метрики:
...
1. Найдите меню над холстом и выберите **Создать конвейер вывода**. Может потребоваться развернуть экран до полного и щелкнуть значок **...** в правом верхнем углу экрана, чтобы найти **в меню Пункт Создать конвейер вывода** .  
...
1. Соедините выходные данные модуля **Ввод данных вручную** с входным *набором данных* (справа) первого модуля **Применить преобразование** .
...
1. Перейдите в **раздел Задания** и выберите последнее задание **Прогнозирование кластеров пингвинов** с именем эксперимента  **mslearn-penguin-inference** .

1. После завершения конвейера щелкните правой кнопкой мыши модуль **Назначение данных кластерам** , выберите **Предварительный просмотр данных** и **набор данных результатов** , чтобы просмотреть прогнозируемые назначения и метрики кластера для трех наблюдений пингвинов во входных данных.
...
1. На странице **Прогнозирование вывода кластеров пингвинов** выберите **Развернуть** в верхней строке меню.

    ![Снимок экрана: кнопка развертывания для конвейера вывода прогноза цены на автомобиль.](media/create-clustering-model/deploy-screenshot.png)
