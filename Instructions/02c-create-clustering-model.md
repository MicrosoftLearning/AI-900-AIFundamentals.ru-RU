...
1. В [Студия машинного обучения Azure](https://ml.azure.com?azure-portal=true) разверните левую панель, выбрав значок меню в левом верхнем углу экрана. Просмотрите страницу конструктора **** (в разделе **"Создание**") и выберите**+**, чтобы создать новый конвейер.

1. Измените имя черновика (**Pipeline-Created-on-date***) на **Обучение кластеризации** пингвинов.
...
1. Вернитесь к конвейеру, выбрав **Конструктор** в меню слева. **На странице конструктора** выберите **проект конвейера "Обучение кластеризации** пингвинов".
...
1. Выберите данные **, найдите и поместите ****набор данных** пингвина на холст.
...
1. Выберите вкладку *"Профиль"* , отметив, что можно увидеть распределения различных столбцов в виде гистограмм.

1. Обратите внимание на следующие характеристики набора данных:

    - Этот набор данных содержит следующие столбцы.
        - **CulmenLength**: Длина счета пингвина в миллиметрах.
        - **CulmenDepth**: Глубина законопроекта пингвина в миллиметрах.
        - **FlipperLength**: Длина липпера пингвина в миллиметрах.
...
1. Закройте страницу **DataOutput** , чтобы просмотреть набор данных на холсте конвейера.

## Применение преобразований

1. **В области библиотеки** активов слева выберите **компонент** (который содержит широкий спектр модулей, которые можно использовать для преобразования данных и обучения модели). Можно также использовать панель поиска для быстрого поиска модулей.
...
1. Нажмите кнопку ****"Сохранить**", а затем закройте меню "Выбрать столбцы" в наборе** данных, чтобы вернуться на холст конструктора.

1. В библиотеке **** активов найдите модуль "Чистые **отсутствующие данные" и поместите его на холст ниже **столбцов в модуле набора** данных** и подключите их следующим образом:
...
1. Дважды щелкните **модуль "Очистить отсутствующие данные** " и в области параметров справа выберите **"Изменить столбец**". Затем в окне **Столбцы для очистки** выберите **С правилами** и добавьте **Все столбцы** следующим образом:
...
1. Нажмите кнопку **"Сохранить** ", а затем в области параметров задайте следующие параметры конфигурации:
    - **Минимальное соотношение** недостающих значений: 0,0
    - **Максимальное число** отсутствующих значений: 1,0
    - **Режим** очистки: удаление всей строки
...
1. Нажмите кнопку **"Сохранить** ", а затем закройте **параметры модуля нормализации данных** , чтобы вернуться на холст конструктора.
...
1. **На странице "Основы"** выберите **"Создать"** и задайте имя эксперимента на **mslearn-пинг-training**, а затем нажмите кнопку **"Далее**".
...
1. Нажмите кнопку **"Рядом",** чтобы просмотреть задание конвейера, а затем нажмите кнопку **"Отправить** ", чтобы запустить конвейер обучения.

1. Дождитесь завершения задания. Это может занять 5 минут или больше. Вы можете проверка состояние задания, выбрав **"Задания**" в разделе **"Активы**". **Выберите задание "Обучение кластеризации** пингвинов".


## Просмотр преобразованных данных

1. После завершения выполнения модули будут выглядеть следующим образом:
...
1. Щелкните правой **кнопкой мыши модуль "Нормализация данных", выберите **"Предварительный просмотр данных****", а затем выберите **"Преобразованный набор** данных", чтобы просмотреть результаты.
...
1. Вернитесь на **страницу конструктора** и откройте проект конвейера **"Обучение кластеризации** пингвинов".
...
1. Модель, для которой выполняется обучение, будет использовать компоненты для группирования данных в кластеры, поэтому нам нужно обучить модель с помощью алгоритма *кластеризации*. **В библиотеке** активов найдите и поместите **модуль кластеризации** K-Средних на холст слева **от модуля разделенных данных** и над **модулем "Обучение модели** кластеризации". После этого подключите область его выходных данных к области входных данных **Необученная модель** (слева) модуля **Модель кластеризации**.
...
1. Алгоритм *K-средние* группирует элементы в заданное число кластеров — это значение называется ***K***. Выберите модуль **Кластеризация K-средних** и на панели справа задайте для параметра **Число центроидов** значение **3**.
    ...
    > 5. Повторяющиеся шаги c. и d. до тех пор, пока выделение кластера не стабилизируется или указанное число итерации завершено.
...
1. Выберите "Настроить" и " **Отправить**" и запустите конвейер с помощью существующего эксперимента с именем **mslearn-penguin-training** в вычислительном кластере.

1. Дождитесь завершения задания. Это может занять 5 минут или больше. Проверьте состояние задания, выбрав **"Задания** " в разделе **"Активы**". Выберите последнюю **работу "Кластеризация** пингвинов".

1. После завершения выполнения эксперимента щелкните правой кнопкой мыши **модуль "Назначение данных кластерам** ", выберите **предварительный просмотр данных** и выберите **набор** данных результатов для просмотра результатов.
...
1. **На странице конструктора** откройте **проект конвейера "Обучение кластеризации** пингвинов".
...
1. Дождитесь завершения эксперимента. Чтобы проверка его состояние, перейдите на **страницу "Задания**" и выберите последнее **задание "Обучение кластеризации** пингвинов".

1. Щелкните правой кнопкой мыши модуль оценки модели, выберите **предварительный **просмотр данных**, а затем выберите **результаты** оценки.** В каждой строке изучите следующие метрики:
...
1. Найдите меню над холстом и выберите " **Создать конвейер** вывода". Возможно, потребуется развернуть экран до полного и щелкнуть **значок ...** в правом верхнем углу экрана, чтобы найти **конвейер** вывода в меню.  
...
1. Переименуйте новый конвейер в **Прогнозирование кластеров данных о пингвинах**, а затем проверьте новый конвейер. Преобразования и модель кластеризации в конвейере обучения являются частью этого конвейера. Обученная модель будет использоваться для оценки новых данных. Конвейер также содержит выходные данные веб-службы для возврата результатов.
    ...
    - Замените **набор данных пингвинов компонентом **Ввод данных** вручную**, который не включает **столбец "Виды**".
    - Удалите компонент выбора столбцов в компоненте **набора** данных, который теперь является избыточным.
    - ** ПодключениеКомпоненты ввода** и **ввода данных веб-службы вручную** (которые представляют входные данные для кластеризации данных) к первому **компоненту "Применить преобразование**".
...
1. Конвейер не включает автоматически компонент **входных данных веб-службы** для моделей, созданных на основе пользовательских наборов данных. Найдите компонент **входных данных веб-службы** в библиотеке ресурсов и поместите его в начало конвейера.  Подключение выходные ** данныеКомпонент ввода** веб-службы в входные *данные набора* данных (справа) первого **компонента применения преобразования**, который уже находится на холсте.  
...
1. Подключение выходные ** данныеВведите модуль DataLy** вручную в входные *данные набора* данных (справа) первого **модуля Apply Transformation**.
...
1. Перейдите к **заданиям** и выберите последнее **задание Predict Penguin Clusters** с  **именем эксперимента mslearn-пинг-inference** .

1. После завершения конвейера щелкните правой кнопкой мыши **модуль "Назначить данные кластерам** ", выберите **"Предварительный просмотр данных** " и выберите **набор** данных "Результаты", чтобы просмотреть прогнозируемые назначения кластера и метрики для трех наблюдений за пингвинами в входных данных.
...
1. **На странице запуска прогноза кластеров пингвинов** выберите **"Развернуть**" в верхней строке меню.
...
1. Выберите **"Развернуть новую конечную точку** в режиме реального времени" и используйте следующие параметры:
    - **Имя**: predict-penguin-clusters
    - **Описание**: кластерные пингвины.
    - **Тип** вычислений: экземпляр контейнера Azure

1. Дождитесь развертывания веб-службы. Это может занять несколько минут. 

1. Чтобы просмотреть состояние развертывания, разверните левую область, выбрав значок меню в левом верхнем углу экрана. Просмотрите страницу **Конечные точки** (в разделе **Ресурсы**) и выберите **predict-penguin-clusters**. По завершении развертывания **состояние развертывания** изменится на **работоспособно**.
